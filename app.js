...
function loadDB() {
  ...
  db.users = (db.users || []).map((user) => {
    ...
    return {
      ...user,
      bio: user.bio || "",
      unavailableDays: Array.isArray(user.unavailableDays) ? user.unavailableDays : [],
    };
  });

  db.catalog = db.catalog
    .map((service) => {
      const hasTieredModel = Array.isArray(service.tiers) && service.tiers.length;
      if (hasTieredModel) return service;
      ...
      return {
        ...
        tiers: [{
          name: "Standard",
          minPrice: fallbackPrice || 5000,
          maxPrice: fallbackPrice || 5000,
          description: "Legacy listing converted to tiered pricing",
        }],
        acceptsNegotiation: service.acceptsNegotiation !== false,
        optimalPrice: Number(service.optimalPrice || fallbackPrice || 0),
      };
    })
    .filter((service) => service && service.providerId);
